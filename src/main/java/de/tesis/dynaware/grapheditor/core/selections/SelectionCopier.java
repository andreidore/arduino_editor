 ...
 * <ol>
 * <li>All selected nodes and their connectors (containment references) are
 * copied.</li>
 * <li>If a connection's source and target nodes are <b>both</b> copied, the
 * connection and its joints are also copied.</li>
 * </ol>
 * </p>
 */
public class SelectionCopier {
	 ...
	 *            the {@link SelectionTracker} instance for the graph editor
	 * @param selectionCreator
	 *            the {@link SelectionCreator} instance for the graph editor
	 * @param selectionDeleter
	 *            the {@link SelectionDeleter} instance for the graph editor
	 */
	 ...
	 * 
	 * @param handler
	 *            a {@link CommandAppender} to allow custom commands to be appended
	 *            to the cut command
	 */
	public void cut(final Consumer<List<GNode>> consumer) {
	...
	public void copy() {
		...
		// Don't iterate directly over selectionTracker.getSelectedNodes() because that
	 ...
	 *
	 * @param consumer
	 *            a consumer to allow custom commands to be appended to the paste
	 *            command
	 * @return the list of new {@link GNode} instances created by the paste
	 *         operation
	 */
	public List<GNode> paste(final Consumer<List<GNode>> consumer) {
	...
	/**
	 * Clears the memory of what was cut / copied. Future paste operations will do
	 * nothing.
	 */
	 ...
	 *
	 * @param pastedNodes
	 *            an empty list to be filled with pasted nodes
	 * @param pastedConnections
	 *            an empty list to be filled with pasted connections
	 */
	...
	/**
	 * Adds an x and y offset to all nodes and connections that are about to be
	 * pasted.
	 *
	 * @param pastedNodes
	 *            the nodes that are going to be pasted
	 * @param pastedConnections
	 *            the connections that are going to be pasted
	 */
	private void addPasteOffset(final List<GNode> pastedNodes, final List<GConnection> pastedConnections) {
		...
		for (final GNode node : pastedNodes) {
			node.getX().set(node.getX().get() + pasteOffset.getX());
			node.getY().set(node.getY().get() + pasteOffset.getY());
		}

		for (final GConnection connection : pastedConnections) {
			for (final GJoint joint : connection.getJoints()) {
				joint.getX().set(joint.getX().get() + pasteOffset.getX());
				joint.getY().set(joint.getY().get() + pasteOffset.getY());
			}
		}
	}

	/**
	 * Checks that the pasted node and joints will be inside the bounds of their
	 * parent.
	 *
	 * <p>
	 * Corrects the x and y positions accordingly if they will be outside the
	 * bounds.
	 * </p>
	 *
	 * @param pastedNodes
	...
	private void checkWithinBounds(final List<GNode> pastedNodes, final List<GConnection> pastedConnections) {
		...
		if (parentAtTimeOfCopy instanceof Region) {
			...
			if (xCorrection != 0 || yCorrection != 0) {
				...
				for (final GNode node : pastedNodes) {
					node.getX().set(node.getX().get() + xCorrection);
					node.getY().set(node.getY().get() + yCorrection);
				}

				for (final GConnection connection : pastedConnections) {
					for (final GJoint joint : connection.getJoints()) {
						joint.getX().set(joint.getX().get() + xCorrection);
						joint.getY().set(joint.getY().get() + yCorrection);
					}
				}
			}
		}
	 ...
	 *
	 * @param pastedNodes
	 *            the pasted nodes to be added
	 * @param pastedConnections
	 *            the pasted connections to be added
	 * @param consumer
	 *            a consumer to allow custom commands to be appended to the paste
	 *            command
	 */
	private void addPastedElements(final List<GNode> pastedNodes, final List<GConnection> pastedConnections,
			final Consumer<List<GNode>> consumer) {

		/*
		 * 
		 * final EditingDomain editingDomain =
		 * AdapterFactoryEditingDomain.getEditingDomainFor(model); final CompoundCommand
		 ...
		 * 
		 * for (final GConnection pastedConnection : pastedConnections) {
		 * command.append(AddCommand.create(editingDomain, model, CONNECTIONS,
		 * pastedConnection)); }
		 * 
		 * if (command.canExecute()) { editingDomain.getCommandStack().execute(command);
		 * }
		 * 
		 * if (consumer != null) { consumer.accept(pastedNodes, command); }
		 */

	}

	/**
	 * Saves the position in the scene of the JavaFX {@link Parent} of the node
	 * skins.
	 */
	...
	/**
	 * Determines the offset by which the new nodes and joints should be positioned
	 * relative to the nodes and joints they were copied from.
	 *
	 * <p>
	 * The aim here is to paste the new nodes and joints into a <b>visible</b> area
	 * on the screen, even if the user has panned around in the graph editor
	 * container since the copy-action took place.
	 * </p>
	 *
	...
	/**
	 * Gets the start and end x- and y-positions of the given region (including
	 * insets).
	 *
	 * @param region
	 *            a {@link Region}
	...
	/**
	 * Gets the start and end x- and y-positions of the given group of nodes and
	 * joints.
	 *
	 * @param nodes
	 *            a list of nodes
	...
	private Bounds getContentBounds(final List<GNode> nodes, final List<GConnection> connections) {
		...
		for (final GNode node : nodes) {
			...
			if (node.getX().get() < contentBounds.startX) {
				contentBounds.startX = node.getX().get();
			}
			if (node.getY().get() < contentBounds.startY) {
				contentBounds.startY = node.getY().get();
			}
			if (node.getX().get() + node.getWidth().get() > contentBounds.endX) {
				contentBounds.endX = node.getX().get() + node.getWidth().get();
			}
			if (node.getY().get() + node.getHeight().get() > contentBounds.endY) {
				contentBounds.endY = node.getY().get() + node.getHeight().get();
			}
		}
		...
		for (final GConnection connection : connections) {
			for (final GJoint joint : connection.getJoints()) {
				...
				if (joint.getX().get() < contentBounds.startX) {
					contentBounds.startX = joint.getX().get();
				}
				if (joint.getY().get() < contentBounds.startY) {
					contentBounds.startY = joint.getY().get();
				}
				if (joint.getX().get() > contentBounds.endX) {
					contentBounds.endX = joint.getX().get();
				}
				if (joint.getY().get() > contentBounds.endY) {
					contentBounds.endY = joint.getY().get();
				}
			}
		}
