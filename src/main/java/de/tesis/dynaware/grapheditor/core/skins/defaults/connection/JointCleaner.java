...
public class JointCleaner {
	...
	/**
	 * Adds handlers to remove all joints that are on top of each other after a
	 * mouse-released gesture.
	 *
	 * @param jointSkins
	 *            the connection's joint skins
	 */
		...
		for (final GJointSkin jointSkin : jointSkins) {
			...
			final EventHandler<MouseEvent> newHandler = event -> {
				...
				if (!jointsToCleanUp.isEmpty()) {
					...
					// AdapterFactoryEditingDomain.getEditingDomainFor(connection);
					// final CompoundCommand command = new CompoundCommand();

					final GModel model = graphEditor.getModel();
					final SkinLookup skinLookup = graphEditor.getSkinLookup();

					JointCommands.removeJoints(jointsToCleanUp, connection);
	...
	/**
	 * Finds the joints that should be cleaned up / removed from a list of joint
	 * positions.
	 *
	 * @param jointPositions
	 *            a list of {@link Point2D} instances containing the x and y values
	 *            of the joints
	...
	/**
	 * Finds the first pair of identical joint positions and removes them from the
	 * list.
	 *
	 * @param jointPositions
	 *            a list of {@link Point2D} instances containing the x and y values
	 *            of the joints
	 * @return the position of the removed joints, or {@code null} if nothing was
	 *         found to remove
	 */
