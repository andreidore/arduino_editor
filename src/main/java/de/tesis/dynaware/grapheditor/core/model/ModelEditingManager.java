...
public class ModelEditingManager {
	 ...
	 *            the {@link CommandStackListener} that listens for changes in the
	 *            model
	 */
	public ModelEditingManager() {
	...
	public void updateLayoutValues(final SkinLookup skinLookup) {
		...
		Commands.updateLayoutValues(model, skinLookup);
	...
	public void remove(final List<GNode> nodesToRemove, final List<GConnection> connectionsToRemove) {
		 ...
		 * command.append(RemoveCommand.create(editingDomain, model, CONNECTIONS,
		 * connection));
		 * 
		 * final GConnector source = connection.getSource(); final GConnector target =
	...
	private void initializeEditingDomain(final GModel oldModel, final GModel newModel) {
		...
		/*
		 * // First remove the listener from the old model, if it exists. if (oldModel
		 * != null) { final EditingDomain oldDomain =
		 * AdapterFactoryEditingDomain.getEditingDomainFor(oldModel); if (oldDomain !=
		 * null) {
		 ...
		 * ComposedAdapterFactory.Descriptor.Registry.INSTANCE; final AdapterFactory
		 * adapterFactory = new ComposedAdapterFactory(registry);
		 * 
		 * editingDomain = new AdapterFactoryEditingDomain(adapterFactory, new
		 * BasicCommandStack());
