...
/**
 * Responsible for deleting a selection of one or more elements in the graph
 * editor.
 */
public class SelectionDeleter {
	...
	/**
	 * Creates a new selection deleter. Only one instance should exist per
	 * {@link DefaultGraphEditor} instance.
	 *
	 * @param skinLookup
	 *            the {@link SkinLookup} used to look up skins
	 * @param modelEditingManager
	 *            the {@link ModelEditingManager} used to make changes to the model
	 */
	 ...
	 * @param consumer
	 *            a consumer to allow custom commands to be appended to the delete
	 *            command
	 */
	public void deleteSelection(final GModel model, final Consumer<List<GNode>> consumer) {
		...
		if (!nodesToDelete.isEmpty() || !connectionsToDelete.isEmpty()) {
			...
			// final CompoundCommand command = modelEditingManager.remove(nodesToDelete,
			// connectionsToDelete);

			if (consumer != null) {
				consumer.accept(nodesToDelete);
			}
		}
	}
}
