...
public class DefaultGraphEditor implements GraphEditor {
	...
	public DefaultGraphEditor() {
		...
		// Skin manager needs 'this' reference so users can access GraphEditor inside
		// their custom skins.
	...
	@Override
	public void setOnConnectionCreated(final Consumer<GConnection> consumer) {
		connectionEventManager.setOnConnectionCreated(consumer);
	}

	@Override
	public void setOnConnectionRemoved(final Consumer<GConnection> consumer) {
		connectionEventManager.setOnConnectionRemoved(consumer);
	}

	/**
	 * Adds a listener to the model property to set the controller value whenever
	 * the model property is updated.
	 */
