package com.setyrobotics.arduinoeditor.skin;
 ...
 */
public class NodeSkin extends GNodeSkin {
	...
	/**
	 * Creates a new {@link NodeSkin} instance.
	 *
	 * @param node
	 *            the {link GNode} this skin is representing
	 */
	...
	public Point2D getConnectorPosition(final GConnectorSkin connectorSkin) {
		...
		if (inputConnectorSkins.contains(connectorSkin)) {
			...
			// Subtract 1 to align start-of-connection correctly. Compensation for rounding
			// errors?
	...
	/**
	 * Lays out the selection halo based on the current width and height of the node
	 * skin region.
	 */
	...
	/**
	 * Adds a listener to react to whether the node is selected or not and change
	...
	/**
	 * Removes any input and output connectors from the list of children, if they
	 * exist.
	 */
	...
	/**
	 * Adds or removes the 'selected' pseudo-class from all connectors belonging to
	 * this node.
	 * 
	 * @param isSelected
	 *            {@code true} to add the 'selected' pseudo-class, {@code false} to
	 *            remove it
	 */
	 ...
	 * 
	 * @param event
	 *            a mouse-dragged event on the node
	 */
